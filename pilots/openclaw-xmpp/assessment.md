...
`phases/swk-03-assessment.md` als durch swk-02 klärbare Wissenslücke, als
Anforderungsklärung durch den Ideengeber oder als echter späterer
...
Die Spalte „Art“ folgt `phases/swk-03-assessment.md` Arbeitsschritt 9 und
`rules/questions.md`: Eine fehlende Information oder eine noch unpräzise Aussage
der `idea.md` ist für sich genommen kein Entscheidungsbedarf.

| Sachverhalt | Art | Bedeutung für Bewertung | Durch swk-02 klärbar? | Behandlung |
|---|---|---|---|---|
| Funktionsfähigkeit der Projekte für 1:1, OMEMO und MUC (`evd-052`) | Wissenslücke | wesentlich; ohne sie bleibt jede Aussage zu Funktionen auf dokumentierte Angaben und vorhandenen Code beschränkt | ja; Build und Test in einer getrennten, entbehrlichen Arbeitsumgebung sind nach `phases/swk-02-inventory.md` zulässig | Rückkehr zu swk-02 (Rückkehrpunkt 1) |
| Ladefähigkeit der Projekte unter 2026.7.1-2 und 2026.9.4 (`evd-053`) | Wissenslücke | wesentlich für den in `idea.md` Abschnitt 1 genannten Versionsbezug | ja; Laden in getrennten OpenClaw-Instanzen der beiden Versionen | Rückkehr zu swk-02 (Rückkehrpunkt 2) |
| Vollständigkeit der Projektliste außerhalb von GitHub und npm (`evd-054`) | Wissenslücke | wesentlich für die in `idea.md` Abschnitt 1 verlangte Untersuchung vorhandener Projekte | ja; Suche auf ClawHub, GitLab und Codeberg | Rückkehr zu swk-02 (Rückkehrpunkt 3) |
| Umfang der geforderten Funktionen, insbesondere OMEMO in Gruppenchats (`idea.md` Abschnitt 10; `evd-025`, `evd-036`, `evd-042`) | Anforderungsklärung | wesentlich; bestimmt, welche Projekte für diesen Aspekt in Betracht kommen | nein; es ist eine Präzisierung der Projektidee, kein Bestand | Klärung mit Ideengeber (Abschnitt 8.1) |
| Bedeutung von „unterstützen“ je Funktion (`idea.md` Abschnitt 10) | Anforderungsklärung | wesentlich für die spätere Abnahme | nein; Präzisierung der Projektidee | Klärung mit Ideengeber (Abschnitt 8.1) |
| Umfang der zu unterstützenden OpenClaw-Versionen (`idea.md` Abschnitt 10; `evd-001`, `evd-006`, `evd-009`) | Anforderungsklärung | wesentlich; bestimmt den Versionsbezug der Bewertung | teilweise; der Standort der zweiten Installation ist nicht ermittelbar, die Versionsangaben selbst sind bereits erfasst | Klärung mit Ideengeber (Abschnitt 8.1) |
| Rolle des eigenen XMPP-Servers sowie Software und Erweiterungen (`evd-002`, `evd-008`) | Anforderungsklärung | wesentlich für die Beurteilung von MUC und OMEMO im Betrieb | nein; Standort und Zugang sind unbekannt und nur vom Ideengeber zu benennen | Klärung mit Ideengeber (Abschnitt 8.1) |
| Zuschnitt des Verfahrens für neue OpenClaw-Versionen, einschließlich KI-Agenten (`idea.md` Abschnitt 10; `evd-010`, `evd-014`, `evd-016`) | Anforderungsklärung | wesentlich für den Umfang des Projektgegenstands | nein; Präzisierung der Projektidee | Klärung mit Ideengeber (Abschnitt 8.1) |
| Erprobte Plugins und Ursachen gescheiterter Installationen (`evd-003`, `evd-007`) | Wissenslücke | wesentlich, um die Aussage aus `idea.md` Abschnitt 2 auf die untersuchten Projekte zu beziehen | nein; das Konfigurationsverzeichnis ist nicht lesbar, es gibt keine Aufzeichnung | Klärung mit Ideengeber (Abschnitt 8.1) |
| Grundlage der Umsetzung: vorhandenes Projekt oder Neuentwicklung (`evd-018`, `evd-020`, `evd-022` bis `evd-051`) | möglicher Entscheidungsbedarf | wesentlich; bestimmt den weiteren Weg des Projekts | nein; die Befundlage ist erhoben, die Auswahl bleibt offen | spätere Phase (`dnd-001`, Abschnitt 8.2) |

## 7. Rückkehrpunkte zu swk-02

Alle drei Rückkehrpunkte sind offen. Nach `phases/swk-03-assessment.md`,
Abschnitt „Rückkehr zu swk-02“, ist die Qualitätsgrenze von swk-03 deshalb
nicht bestanden, und `assessment.md` darf den Status `review` nicht erhalten.
...
- offene Klärung: Präzisierung der in `idea.md` genannten Funktionen durch den Ideengeber; die Befunde zeigen Projekte mit OMEMO nur für Direktnachrichten, mit OMEMO auch bei Gruppenchats und ohne OMEMO.
- Auswirkung auf spätere Bewertung oder Entscheidung: Ohne diese Präzisierung lässt sich nicht bestimmen, welche Projekte den Aspekt abdecken; sie geht der Entscheidung `dnd-001` voraus.

#### Umfang der zu unterstützenden OpenClaw-Versionen

- Gegenstand: Welche Versionen „aktuelle OpenClaw-Versionen“ umfassen und ob beide in `idea.md` Abschnitt 2 genannten Installationen unterstützt werden sollen.
...
- offene Klärung: Festlegung des Versionsbezugs durch den Ideengeber. Die Schnittstelle unterscheidet sich zwischen den beiden genannten Versionen (`evd-011`, `evd-017`), die Projekte deklarieren unterschiedliche Untergrenzen (`evd-029`, `evd-032`, `evd-033`, `evd-035`, `evd-037`, `evd-039`, `evd-042`, `evd-044`).
- Auswirkung auf spätere Bewertung oder Entscheidung: Der Versionsbezug bestimmt, welche Projekte nach ihren deklarierten Angaben in Betracht kommen, und wirkt damit auf `dnd-001` und auf Rückkehrpunkt 2.

#### Rolle des eigenen XMPP-Servers

- Gegenstand: Ob das Plugin mit dem in `idea.md` Abschnitt 2 genannten eigenen XMPP-Server betrieben werden soll und welcher Server das ist.
...
- offene Klärung: Benennung des Servers durch den Ideengeber; Software, Standort und unterstützte Erweiterungen sind unbekannt und aus dem Bestand nicht ermittelbar.
- Auswirkung auf spätere Bewertung oder Entscheidung: Ohne diese Angabe bleibt offen, welche serverseitigen Voraussetzungen für MUC und OMEMO gelten; erst danach ist eine Bewertung dieses Punktes möglich.
...
- offene Klärung: Präzisierung des „oder“ und des Stellenwerts der KI-Agenten durch den Ideengeber; die Befunde belegen nur den Änderungsbedarf an der Schnittstelle, nicht den gewünschten Umfang.
- Auswirkung auf spätere Bewertung oder Entscheidung: Der Umfang des Projektgegenstands ist ohne diese Klärung nicht bestimmt; eine Auswahl zwischen dokumentierten Optionen steht noch nicht an.

#### Erprobte Plugins und Ursachen der gescheiterten Installationen

- Gegenstand: Welche Plugins der Ideengeber erprobt hat und woran ihre Installation scheiterte.
- Bezug zur `idea.md`: Abschnitt 2, Abschnitt 11
- relevante Befunde: `evd-003`, `evd-007`
- offene Klärung: Angabe durch den Ideengeber; das Konfigurationsverzeichnis ist nicht lesbar, und es gibt keine Aufzeichnung (`evd-007`).
- Auswirkung auf spätere Bewertung oder Entscheidung: Ohne diese Angabe lässt sich die Aussage aus `idea.md` Abschnitt 2 nicht auf die untersuchten Projekte beziehen.
...
- Begründung des Entscheidungsbedarfs: Der Hersteller liefert keine XMPP-Anbindung (`evd-018`, `evd-019`, `evd-020`). Es liegen dreizehn erfasste Projekte mit unterschiedlicher Abdeckung vor (Abschnitt 5). Für den weiteren Prozess ist eine Auswahl zwischen dokumentierten Handlungsoptionen erforderlich; sie ist nach `phases/swk-03-assessment.md` nicht Teil dieser Phase.
...
- offene entscheidungsrelevante Sachverhalte: Funktionsfähigkeit (`evd-052`, Rückkehrpunkt 1), Ladefähigkeit (`evd-053`, Rückkehrpunkt 2), Vollständigkeit der Projektliste (`evd-054`, Rückkehrpunkt 3), Lizenzlage der Projekte (in `inventory.json` nur als Quellenangabe erfasst, nicht als Befund); vorgelagert sind die Anforderungsklärungen in Abschnitt 8.1.
...
|---|---|---|
| `idea.md` vorhanden und `accepted` | ja | `pilots/openclaw-xmpp/idea.md` Zeile 5: `status: accepted`; Freigabe in Abschnitt 13 vom 2026-09-14 |
| `inventory.json` vorhanden und `accepted` | ja | `metadata.status` = `accepted`; `approval` vom 2026-09-16 durch Martin Henkel |
| `inventory.md` vorhanden und aktuell erzeugt | ja | `python3 tools/inventory.py check pilots/openclaw-xmpp/inventory.json pilots/openclaw-xmpp/inventory.md` meldet „current“ |
| vorhandene `assumptions.md` berücksichtigt | nicht vorhanden | keine Datei `pilots/openclaw-xmpp/assumptions.md`; `idea.md` Abschnitt 11 nennt keine erkannten Annahmen |
| referenzierte Regeln und Vorlagen vorhanden und verwendbar | ja | `phases/swk-03-assessment.md`, `rules/process.md`, `rules/evidence.md`, `rules/assumptions.md`, `rules/questions.md`, `rules/identifiers.md`, `rules/status.md`, `rules/quality-gates.md`, `templates/assessment.md` vorhanden und nicht leer |
| Qualitätsgrenze swk-03 vorhanden | ja | `rules/quality-gates.md`, Abschnitt „Qualitätsgrenze swk-03“ |
| Kennungen der Eingaben entsprechen `rules/identifiers.md` | ja | `evd-001` bis `evd-054` kleingeschrieben und lückenlos; `tools/inventory.py validate` meldet „valid“ |
...
- offene erforderliche Rückkehrpunkte zu swk-02: ja (Rückkehrpunkt 1, 2 und 3)
- nicht erfüllte Kriterien:
  - „kein erforderlicher Rückkehrpunkt zu `swk-02` mehr offen ist“ (`rules/quality-gates.md`, Qualitätsgrenze swk-03): drei Rückkehrpunkte sind offen.
- Blockade: keine Blockade im Sinne von `rules/status.md`. Die benötigten Untersuchungen sind in swk-02 durchführbar; die Bearbeitung ist nicht dauerhaft verhindert. Nach `rules/quality-gates.md`, Abschnitt „Nichtbestehen der Qualitätsgrenze“, bleibt das Dokument deshalb auf `draft`.

Erfüllte Kriterien der Qualitätsgrenze, soweit ohne die offenen Rückkehrpunkte
prüfbar:
...
- die Gegenüberstellung in Abschnitt 5 verwendet für alle Alternativen dieselben fünf Bewertungsaspekte; die Aufteilung auf drei Tabellen ändert die Kriterienbasis nicht
...
- Wissenslücken, Anforderungsklärungen und Entscheidungsbedarfe sind nach `rules/questions.md` getrennt; nur `dnd-001` ist als echter Entscheidungsbedarf geführt
- keine nicht definierte Kennungsart verwendet; die zuvor verwendeten freien Kennungen für Entscheidungsbedarf sind entfallen
...
- Anmerkungen: Die Qualitätsgrenze swk-03 ist nicht bestanden, weil die Rückkehrpunkte 1 bis 3 zu swk-02 offen sind. Das Dokument bleibt nach `rules/quality-gates.md` auf `draft`. Vor einer erneuten Prüfung sind die Rückkehrpunkte in swk-02 zu bearbeiten; die Anforderungsklärungen in Abschnitt 8.1 benötigen Angaben des Ideengebers.
